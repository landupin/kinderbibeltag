...
func anmelden(w http.ResponseWriter, r *http.Request) {
	...
	fmt.Println("submitted: ", child)

	//form validation
	berr, validationErrors := formValidations(child, r)
	//error handling of form validation
	if berr {
		d := Data{
			Kibitagsets:      kibitagset,
...
func formValidations(child formKind, r *http.Request) (bool, formKind) {
	err := false
	...
	if child.Essen == "0" || child.Essen == "" {
		...
		err = true
	} else if kibitagset.Mitbringen[indess].Anzahl <= 0 {
